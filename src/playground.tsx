...
export function Playground() {
  ...
  const wheelTheme = useMemo<WheelThemeOptions>(() => ({
    ...
    text: {
      ...
      innerRadius: themeSettings.textInnerRadius,
      ...
      minFontSize: themeSettings.minFontSize,
      maxLines: themeSettings.maxLines,
    },
  }), [themeSettings]);
      ...
      <div className="playgroundGrid">
        ...
        <aside className="controls">
          ...
          <section className="controlSection">
            ...
            <label className="toggleLine"><input type="checkbox" checked={debug} onChange={(event) => setDebug(event.target.checked)} /> {copy.appearance.debug}</label>
            ...
            <details className="controlDisclosure">
              ...
              <div className="disclosureBody">
                ...
                <div className="twoColumns topField">
                  ...
                  <label className="selectField">{copy.theme.overflow}<select value={themeSettings.overflow} onChange={(event) => updateTheme('overflow', event.target.value as SectorTextStyle['overflow'])}><option value="hide">{copy.theme.hide}</option><option value="ellipsis">{copy.theme.ellipsis}</option><option value="shrink">{copy.theme.shrink}</option><option value="shrink-wrap">{copy.theme.shrinkWrap}</option></select></label>
                </div>
                ...
                <div className="twoColumns topField">
                  <label className="rangeField compactRange">{copy.theme.innerRadius} <output>{themeSettings.textInnerRadius.toFixed(2)}</output><input type="range" min="0" max=".9" step=".01" value={themeSettings.textInnerRadius} onChange={(event) => updateTheme('textInnerRadius', Number(event.target.value))} /></label>
                </div>
                {themeSettings.overflow === 'shrink-wrap' && <div className="twoColumns topField">
                  <label className="numberField">{copy.theme.minSize}<input type="number" min=".5" max="10" step=".1" value={themeSettings.minFontSize} onChange={(event) => updateTheme('minFontSize', Number(event.target.value))} /></label>
                  <label className="numberField">{copy.theme.maxLines}<input type="number" min="1" max="6" step="1" value={themeSettings.maxLines} onChange={(event) => updateTheme('maxLines', Number(event.target.value))} /></label>
                </div>}
              </div>
            </details>
            <details className="controlDisclosure">
          ...
          <section className="controlSection">
            ...
            {selectedItem && items.length <= 40 && selectedText && <div className="sectorTextEditor">
              ...
              <div className="threeColumns topField"><label className="selectField">{copy.theme.font}<select value={selectedText.fontFamily} onChange={(event) => updateItemText(selectedItem.id, { fontFamily: event.target.value })}><option value="Inter, ui-sans-serif, system-ui, sans-serif">Inter / system</option><option value="Georgia, serif">Georgia</option><option value="monospace">Monospace</option></select></label><label className="selectField">{copy.theme.orientation}<select value={selectedText.orientation} onChange={(event) => updateItemText(selectedItem.id, { orientation: event.target.value as SectorTextStyle['orientation'] })}><option value="tangential">{copy.theme.tangential}</option><option value="radial">{copy.theme.radial}</option><option value="horizontal">{copy.theme.horizontal}</option></select></label><label className="selectField">{copy.theme.overflow}<select value={selectedText.overflow} onChange={(event) => updateItemText(selectedItem.id, { overflow: event.target.value as SectorTextStyle['overflow'] })}><option value="hide">{copy.theme.hide}</option><option value="ellipsis">{copy.theme.ellipsis}</option><option value="shrink">{copy.theme.shrink}</option><option value="shrink-wrap">{copy.theme.shrinkWrap}</option></select></label></div>
              {selectedText.overflow === 'shrink-wrap' && <div className="twoColumns topField"><label className="numberField">{copy.theme.minSize}<input type="number" min=".5" max="10" step=".1" value={selectedText.minFontSize ?? themeSettings.minFontSize} onChange={(event) => updateItemText(selectedItem.id, { minFontSize: Number(event.target.value) })} /></label><label className="numberField">{copy.theme.maxLines}<input type="number" min="1" max="6" step="1" value={selectedText.maxLines ?? themeSettings.maxLines} onChange={(event) => updateItemText(selectedItem.id, { maxLines: Number(event.target.value) })} /></label></div>}
              ...
              <div className="twoColumns topField"><label className="rangeField compactRange">{copy.theme.innerRadius} <output>{(selectedText.innerRadius ?? themeSettings.textInnerRadius).toFixed(2)}</output><input type="range" min="0" max=".9" step=".01" value={selectedText.innerRadius ?? themeSettings.textInnerRadius} onChange={(event) => updateItemText(selectedItem.id, { innerRadius: Number(event.target.value) })} /></label></div>
                </div>
              </details>
            </div>}
          </section>
